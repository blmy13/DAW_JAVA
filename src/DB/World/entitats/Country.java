...
@NamedQuery(name = "Country.findCapital", query = "SELECT c.capital.name FROM Country c WHERE c.name = :nom")
@NamedQuery(name = "Country.findByName", query = "SELECT c FROM Country c WHERE c.name = :nom")
@NamedQuery(name = "Country.countAll", query = "SELECT COUNT(c) FROM Country c")
@NamedQuery(name = "Country.sumPopulation", query = "SELECT SUM(c.population) FROM Country c")
@NamedQuery(name = "Contry.getIdAndName", query = "SELECT c.id, c.name FROM Country c")
@NamedQuery(name = "Country.getEuropePopulationRank", query = "SELECT NEW com.objectdb.world.Country(c.name, c.population) FROM COUNTRY c WHERE c.location LIKE '%Europe%' ORDER BY c.population DESC")
@NamedQuery(name = "Country.getRegionsFiveCountries", 
    query = "SELECT NEW DB.World.entitats.RegionsFiveCountriesDTO(c.location) " +
            "FROM Country c " +
            "GROUP BY c.location " +
            "HAVING COUNT(c) > 5")/**
